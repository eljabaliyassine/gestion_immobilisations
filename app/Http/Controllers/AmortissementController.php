...
class AmortissementController extends Controller
{
     ...
     * MODIFIÉE pour inclure les champs comptecompta
     */
    private function regenererPlanAmortissement($dossierId, $dateDerniereCloture, $dateProchaineCloture)
    {
        ...
        // MODIFICATION: Ajout des champs comptecompta
        ...
        foreach ($immobilisations as $index => $immo) {
            Log::info("Immobilisation #{$index}", [
                ...
                'comptecompta_immobilisation_id' => $immo->comptecompta_immobilisation_id,
                'comptecompta_amortissement_id' => $immo->comptecompta_amortissement_id,
                'comptecompta_dotation_id' => $immo->comptecompta_dotation_id
            ]);
        }
        ...
        foreach ($immobilisations as $immobilisation) {
            try {
                ...
                // MODIFICATION: Ajouter les données à insérer avec les champs comptecompta
                $donnees[] = [
                    ...
                    // AJOUT des champs comptecompta
                    'comptecompta_immobilisation_id' => $immobilisation->comptecompta_immobilisation_id,
                    'comptecompta_amortissement_id' => $immobilisation->comptecompta_amortissement_id,
                    'comptecompta_dotation_id' => $immobilisation->comptecompta_dotation_id,
                    'created_at' => now(),
                    'updated_at' => now()
                ];
        ...
        if (!empty($donnees)) {
            try {
                ...
                Log::info("Insertion réussie des plans d'amortissement avec champs comptecompta");
            } catch (\Exception $e) {
