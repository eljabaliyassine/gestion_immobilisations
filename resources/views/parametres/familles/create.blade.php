        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <span>{{ __('Création d\'une famille') }}</span>
                    <a href="{{ route('parametres.familles.index') }}" class="btn btn-sm btn-secondary">Retour à la liste</a>
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route('parametres.familles.store') }}">
                        @csrf
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <input id="libelle" type="text" class="form-control @error('libelle') is-invalid @enderror" name="libelle" value="{{ old('libelle') }}" required autocomplete="libelle">
                                @error('libelle')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="col-md-6">
                                <select id="comptecompta_immobilisation_id" class="form-control @error('comptecompta_immobilisation_id') is-invalid @enderror" name="comptecompta_immobilisation_id" required>
                                    ...
                                    @foreach($comptesCompta as $compte)
                                        <option value="{{ $compte->id }}" {{ old('comptecompta_immobilisation_id') == $compte->id ? 'selected' : '' }}>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <select id="comptecompta_amortissement_id" class="form-control @error('comptecompta_amortissement_id') is-invalid @enderror" name="comptecompta_amortissement_id" required>
                                    <option value="">Sélectionnez un compte</option>
                                    @foreach($comptesCompta as $compte)
                                        <option value="{{ $compte->id }}" {{ old('comptecompta_amortissement_id') == $compte->id ? 'selected' : '' }}>
                                            {{ $compte->numero }} - {{ $compte->libelle }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <select id="comptecompta_dotation_id" class="form-control @error('comptecompta_dotation_id') is-invalid @enderror" name="comptecompta_dotation_id" required>
                                    <option value="">Sélectionnez un compte</option>
                                    @foreach($comptesCompta as $compte)
                                        <option value="{{ $compte->id }}" {{ old('comptecompta_dotation_id') == $compte->id ? 'selected' : '' }}>
                                            {{ $compte->numero }} - {{ $compte->libelle }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <input id="duree_amortissement_par_defaut" type="number" min="1" max="50" class="form-control @error('duree_amortissement_par_defaut') is-invalid @enderror" name="duree_amortissement_par_defaut" value="{{ old('duree_amortissement_par_defaut') }}" autocomplete="duree_amortissement_par_defaut">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <select id="methode_amortissement_par_defaut" class="form-control @error('methode_amortissement_par_defaut') is-invalid @enderror" name="methode_amortissement_par_defaut">
                                    ...
                                    <option value="lineaire" {{ old('methode_amortissement_par_defaut') == 'lineaire' ? 'selected' : '' }}>Linéaire</option>
                                    <option value="degressif" {{ old('methode_amortissement_par_defaut') == 'degressif' ? 'selected' : '' }}>Dégressif</option>
                                </select>
